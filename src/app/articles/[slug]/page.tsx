...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Chaque ouverture de l'article compte comme une vue, affichée pour valoriser l'audience du texte.
  const { views } = await db.article.update({ where: { id: article.id }, data: { views: { increment: 1 } }, select: { views: true } });
